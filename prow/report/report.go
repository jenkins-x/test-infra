...
func reportStatus(ghc GithubClient, pj kube.ProwJob, childDescription string) error {
	...
	if pj.Spec.Report {
		...
		if contextState == kube.TriggeredState {
			contextState = kube.PendingState
		}
		...
		if err := ghc.CreateStatus(refs.Org, refs.Repo, refs.Pulls[0].SHA, github.Status{
			...
			return fmt.Errorf("error creating status state: %s description: %s context: %s URL %s for %s/%s:%s : %v",
				string(contextState),
				description,
				pj.Spec.Context,
				pj.Status.URL,
